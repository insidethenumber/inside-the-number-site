...
**Post-flight verification (`itn-deadline-check-weekday`, ~10:18 AM CDT run):** confirmed via Beehiiv /posts that "Georgia Tech Opened at -290. It's Already Down to -238." shows status Published, Published on Thu, Sep 3, 2026 10:15 AM CDT — matches the recovery row above. No further action needed; not messaging Chuck since the miss and recovery were already reported.

| 2026-09-05 | Sat | 7:45 AM | Scheduled 7:45 AM CDT | YES, on target, main routine | Weekend routine (`itn-daily-weekend`) ran clean end to end, unattended. Beehiiv session was live (no login redirect). Idempotency checked at Step 0 (nothing published/scheduled for today) and re-checked immediately before building the issue — both clean. **AUDIT — Sports: CFB 2 · MLB 1 · UFC 1 (4 total, none over the 2-per-sport cap). Bet types: spread 2 (CFB) · moneyline 2 (MLB, UFC) — 2 of 4 (50%, at the cap, not over). Football-first applied: CFB led as the free pick (no NFL today, NFL opens later this month) — full board surveyed live via ESPN scoreboard read through a browser tab (sandbox still returns `Tunnel connection failed: 403 Forbidden` for site.api.espn.com; unfixed, same as every prior run) and via the-odds-api.com for UFC (ESPN carries no MMA odds at all). Board: 68 CFB games, 15 MLB games, 1 UFC Fight Night card (14 bouts), 0 NFL/NBA/CBB/NHL/PGA (NBA/CBB/NHL off-season, PGA's next event is Sep 17-20).** **Card:** free pick Nevada +1.5 (-118) vs Western Kentucky, 10:30 PM ET — Nevada opened +4.5/+120 on the moneyline and closes +1.5/-112, with the moneyline flipping outright (Western Kentucky -108 now); de-vigged that's a 50.4%-49.6% game, about a true coin flip, so the free 1.5 points stood out. Also UCLA -2.5 (-112) at Cal — Cal opened -1.5 and closed +2.5, a four-point swing that also flipped the moneyline (Cal -115→+114, UCLA -105→-135); de-vigged UCLA is 55.1% to Cal's 44.9%, total held at 53.5 throughout. Also Rays ML (-110) at Texas — Drew Rasmussen (14-5, 2.95 ERA) against Jacob deGrom (10-9, 4.00 ERA) at a dead-even -110/-110. Also Klaudia Syguła -115 over Nora Cornolle on the UFC Fight Night card (both sides priced at minus money; de-vigged 51.1%/48.9%). Biggest overnight move (not a pick): Charlotte -10.5 → -20.5 over The Citadel, a 10-point swing, moneyline -535 → -1650, total flat at 46.5 the whole time — the largest move on the board by a wide margin. **Stale-game check:** free pick and UCLA/Cal both kick 10:30 PM ET (10 AM CT lead time before send); Rays/Rangers 7:05 PM ET; UFC pick 1:00 PM ET — all comfortably after the 7:45 AM CT send, nothing presented as bettable that had already started. **Site:** `index.html` `todaysGames` (4 entries, exactly one `free:true`) and the Pick of the Day card updated to the free pick; the stale Sep-4 "Parlay of the Day" teaser paragraph in the Pick of the Day card was removed rather than left showing yesterday's games — today's parlay was never built (`data/parlays.json` has no Sep-5 entry; out of scope for this routine) and a placeholder claim about games would have been a stale/false claim on the card. `cfb.html` PICK block replaced with both CFB calls (Nevada +1.5, UCLA -2.5) and the stamp date line updated; markers kept intact. Live-site verify: bounded at two cachebust checks — first still served the old San José State/Eastern Michigan card, second (about 15s later via a raw `fetch`) confirmed "Western Kentucky @ Nevada" live. **Git:** fresh `mktemp -d` clone for the site push and this log row, never the shared working copy; credential helper and `itn-secrets.env` copied to a space-free temp path first (same wrinkle as every prior run — the helper cannot run from a path containing spaces). **Beehiiv mechanics:** duplicated the most recent published post ("California Opened a Favorite...", Fri Sep 4), cleared the body via a real mouse click at the seam + shift-cmd-ArrowDown + Delete, then filled it via the documented synthetic-paste method (DataTransfer text/plain + ClipboardEvent into `.ProseMirror`). One paste artifact: a line starting "6. " was auto-converted to a numbered list by an input rule; caught on the screenshot pass, converted back to plain text and rewritten as "6 — " to avoid retriggering the rule. Title/subtitle set via the native-setter trick; the title reverted to the duplicate's old title once after the first clear+set (unexplained — held fine on the second attempt with a focus() call added and a manual blur-click to confirm). All 5 section headers set to Heading 2 individually via the block-type dropdown (no bulk/markdown heading support found in this editor on paste). Author confirmed as ITN Desk only (Authors: 1, no personal account attached). Auto-linked `insidethenumber.com/games` and `x.com/thenumberdesk` both verified resolving to the correct https URLs before scheduling. Scheduled (not published) via Review → Schedule → "Pick a specific time" → typed `today at 7:45am` → echo confirmed "Sep 5, 2026 7:45 AM CDT" → Tab → Schedule button → confirmed "Publishes in 1 hour · Saturday, Sep 5, 2026 7:45 AM CDT" → re-verified on /posts showing status **Scheduled, Sep 5th, 2026 7:45 AM CDT**. No rule went unmet; nothing forced for quota.

**Post-flight verification (`itn-deadline-check-weekend`, ~8:14 AM CDT run):** confirmed via Beehiiv /posts that "Nevada +1.5, But the Moneyline Says 50-50" shows status **Published**, "Published on Sat, Sep 5, 2026 7:45 AM CDT" — exactly on target, no latency. No further action needed; not messaging Chuck since this was a clean on-time send.
| 2026-09-05 (Sat, extra) | Manual publish 12:01 PM CDT by Claude at Chuck's request — Sunday Sep 6 preview issue in the NEW FORMAT (header band, price card, ticker GIF, number card; 4 images verified in editor). Sent to 4 subscribers; rpruette@gmail.com unsubscribed first at Chuck's direction (Inactive, not deleted — Resubscribe available). Heading "Also on the card" → "OTHER CONFIDENT PLAYS". Post 0b625303. |
| 2026-09-07 | Mon (Labor Day) | 7:45 AM | Scheduled 7:45 AM CDT | YES, on target, main routine | Weekend/holiday routine (`itn-daily-weekend`) ran clean end to end, unattended, gated in by the Labor Day holiday exception (day-of-week is Monday, a regular weekday belongs to `itn-daily-weekday`). Beehiiv session was live (no login redirect). Idempotency checked at Step 0 and re-checked immediately before building — both clean, nothing published/scheduled for today. **AUDIT — Sports: CFB 1 · MLB 2 (3 total, neither sport over the 2-per-sport cap). Bet types: spread 1 (CFB) · total 1 (MLB) · spread 1 (MLB run line) — 0 of 3 moneylines (0%, well under the 50% cap). Football-first applied: CFB led as the free pick, the only football on the board today (NFL's 2026 season hasn't opened yet).** Full board surveyed live via ESPN scoreboard JSON read through a browser tab (sandbox still returns `Tunnel connection failed: 403 Forbidden` for site.api.espn.com — unfixed, same as every prior run) plus the ESPN golf leaderboard page for PGA. Board: 11 MLB games, 1 CFB game (SMU @ Florida State, Labor Day night on ESPN), 0 NFL/NBA/CBB/NHL/UFC (all off-season or not yet started), 0 PGA (next event Sep 17-20 in Asheville, "No Tournament Data Available" today). **Card:** free pick SMU -3 (-105) at Florida State, 7:30 PM ET — SMU opened -2.5/-144 and closed -3/-155 while the total climbed 52.5 to 53.5, spread/price/total all moving the same direction; de-vigged roughly 49-51. Also Under 8 (-106) Guardians @ Orioles, 1:35 PM ET — total fell a full run overnight (9 to 8), de-vig ~51-49 toward the under. Also Milwaukee -1.5 (+163) vs Cubs, 2:10 PM ET — run line flipped a full point (open MIL +1.5, close MIL -1.5), de-vigged fair ~36.4% to cover. Biggest overnight move (not a pick): Twins at Tigers — Detroit's run line flipped -1.5 to +1.5 while its moneyline loosened -134 to -120, the largest move on the board. **Stale-game check:** SMU/FSU kicks 7:30 PM ET, Guardians/Orioles 1:35 PM ET, Cubs/Brewers 2:10 PM ET — all well after the 7:45 AM CT send, nothing presented as bettable that had already started. **Site:** `index.html` `todaysGames` array replaced with today's 3 entries (exactly one `free:true`, matching the newsletter's free pick) and the Pick of the Day card updated to SMU -3; the stale "RESULT — Apple Cup" recap paragraph from the prior card was dropped per the no-results-recap rule rather than carried forward. `cfb.html` PICK block was left untouched — that step is scoped to Saturdays in the task prompt and today is Monday. **Git:** fresh `mktemp -d` clone for the site push, the newsletter-assets push, and this log row, never the shared working copy; credential helper and `itn-secrets.env` copied to a space-free temp path first (same wrinkle as every prior run). **Newsletter assets — images: 4 (beehiiv-hosted: yes, GATE 2 passed).** `python3 scripts/newsletter_assets.py` produced header.jpg/pick.jpg/move.gif/number.jpg + issue.html; all 4 confirmed HTTP 200 on insidethenumber.com after push+propagation. **Beehiiv mechanics — one new finding, logged for future runs:** duplicated the most recent published post, pasted `issue.html` via the documented synthetic ClipboardEvent (text/html) into `.ProseMirror` — this correctly triggered beehiiv's own paste-ingestion pipeline and re-uploaded all 4 images to `beehiiv-images-production.s3.amazonaws.com` (GATE 2 verified). However the paste left the OLD duplicated post's entire body appended after the new content (a worse case of the previously-documented "leftover paragraph" quirk — the whole prior issue survived, not just a fragment), and separately the editor's own image-ingestion pipeline transiently produced a duplicate header image. **Both were first fixed with raw DOM `node.remove()` calls, which looked correct on screen and passed every check in the moment, but did NOT persist — a full reload of the editor showed the stale trailing content’s images gone (that part held) but the duplicate header image back and alt text reset to empty, proving the raw-DOM approach does not reliably survive beehiiv's save/sync cycle.** Fix that verified stable across a full reload: read `document.querySelector('.tiptap.ProseMirror').editor.getJSON()`, filter/edit the actual Tiptap document model (removed the duplicate `imageBlock` node by its asset id, set `alt` text on all 4 remaining image nodes), then `editor.commands.setContent(json, true)` to write it back through the real editor transaction path. Re-verified after a hard page reload: 4 images, all `isUploaded:true`, alt text present, 42 content nodes, no stale text, sections and closing disclaimer all present. **Recommendation for future runs:** prefer `editor.getJSON()` + `editor.commands.setContent()` over raw DOM node removal for any post-paste cleanup — DOM surgery is not reliably saved. Title set via real typing (triple-click, cmd+a, type) on both Compose and the Email step's Subject line (which lagged behind the title change, same known quirk); subtitle/preview text also set by typing. Author confirmed ITN Desk only. Test email sent successfully via the Email step's overflow menu. Web-step post thumbnail was still showing a stale image from a prior issue (auto-regenerate did not pick the new header on the first try); fixed by opening the thumbnail picker and manually selecting the newest "Monday, September 7" header asset — SEO meta title/description picked up the new title/subtitle correctly on their own. Scheduled (not published) via Review → Schedule → "Pick a specific time" → typed `today at 7:45am` → echo confirmed "Sep 7, 2026 7:45 AM CDT" → Tab → Schedule button → confirmed "Publishes in 1 hour · Monday, Sep 7, 2026 7:45 AM CDT" → re-verified on /posts showing status **Scheduled, Sep 7th, 2026 7:45 AM CDT**. No rule went unmet; nothing forced for quota.

**Post-flight verification (`itn-deadline-check-weekend`, ~8:14 AM CDT run):** confirmed via Beehiiv /posts that "SMU -3 in Tallahassee, and a Run Line That Flipped Twice" shows status **Published**, "Published on Mon, Sep 7, 2026 7:45 AM CDT" — exactly on target, no latency. No further action needed; not messaging Chuck since this was a clean on-time send.

| 2026-09-08 | Tue | 10:00 AM | **MISSED — published 1:23 PM CDT as a manual recovery at Chuck's direction** | NO, main routine AND both safety nets failed silently | **The worst weekday failure so far: three separate runs produced nothing at all.** `itn-daily-weekday` fired 8:06 AM, `itn-deadline-check-weekday` fired 9:13 and 10:13. None of the three scheduled a post, published a post, wrote `DRAFT_2026-09-08.md`, pushed a site card, appended a SEND_LOG row, or messaged Chuck. Chuck noticed the missing issue himself at ~1:05 PM. **Transcript evidence (read via session_info):** the 8:06 weekday run got through STEP 0 (Beehiiv session live, nothing published/scheduled), read context, started research, made two Chrome `navigate` calls — then the session simply ends. No error, no final message. The 9:13 safety net got further: it pushed the site, began building in Beehiiv, switched from browser automation to the beehiiv MCP ("This is much better than browser automation"), called `list_publications` — then that session also simply ends. Both died mid-turn rather than at any decision gate, which points at the run being cut off (budget/turn limit) rather than a logic bug in the prompt. **This is consistent with, and now explains, the long-standing pattern this log already flagged: reliable on weekends, unreliable on weekdays.** The weekday prompt is far longer and mandates far more verification work than the weekend one; the weekend job fits in a run and the weekday job apparently does not. RECOMMENDED FIX (not yet applied): cut the weekday routine down — move research to a pre-built artifact the run only reads, and make the safety net's FIRST action a message to Chuck rather than its last. **Recovery (manual, in-session, Chuck chose "send one this afternoon anyway"):** built `issue.json` from a fresh ESPN pull, ran `newsletter_assets.py` (4 images OK), pushed assets. **NEW BLOCKER FOUND: Cloudflare Pages deploys are stuck.** `board/mk-tue.html` pushed 12:28 PM was still 404 at 1:15 PM, and `assets/newsletter/2026-09-08/*` never went live at all, while `2026-09-07/header.jpg` returns 200 — so publishing was fine an hour earlier and then stopped. Had the images been pasted from insidethenumber.com they would all have 404'd and the email would have gone out text-only, the exact failure Chuck called out on Sep 6. **Workaround that fixed it:** the GitHub repo is public, so the img srcs were rewritten to `raw.githubusercontent.com/...` before pasting; beehiiv's paste handler fetched those fine and re-hosted all four on its own CDN. (`save_image` via the beehiiv MCP was tried first and is plan-gated — "not available on your current plan".) **GATE 1: 4 images in body. GATE 2: PASSED — 4/4 on beehiiv-images-production, 0 external.** Two known quirks both recurred and were fixed via `editor.getJSON()` + `setContent()`: the whole of yesterday's issue survived the paste (truncated at the node boundary, nodes 47+) and a duplicate header image appeared (dropped node 0, the older `?t=1788780671` asset, keeping today's `?t=1788891542`). Web thumbnail was also stale (Sep 7 header) and was manually repointed to the Sep 8 header. **AUDIT — Sports: MLB 2. Bet types: total 2 · moneyline 0 (0%, well under the 50% cap). FOOTBALL FIRST: not applicable — zero football on today's board (NFL Week 1 opens Wed Sep 9, no CFB midweek).** **RULE NOT MET, stated openly:** the 3-4 pick minimum could not be met without breaching the 2-per-sport cap on an MLB-only board, so the card carries 2 picks. **Card:** free pick Under 8, Twins at Tigers (-105), 6:40 PM ET — total opened 8.5 and is 8 while the under's price fell -115 to -105, the number and the price moving the same way; Detroit's own price firmed -129 to -137. Also Under 7.5, Angels at Red Sox (-104), 6:45 PM ET — total 8 to 7.5, under -118 to -104, same shape. Biggest overnight move: Philadelphia -156 to -143 (Houston +129 to +119). Number to know: -340, the Yankees tonight (opened -314). Market section taught cents-vs-probability: NYY moved 26 cents for ~1.5 points of win chance, PHI moved 13 cents for ~2.1 points. **Stale-game check:** first pitch on the whole board is 6:35 PM ET, roughly five hours after the 1:23 PM CDT send. Nothing in play was presented as bettable. **Published to Email and Web, all 5 subscribers, bylined ITN Desk, 577 words.**

| 2026-09-09 | Wed | 10:00 AM | Scheduled 10:00 AM CDT (pre-flight save) | Main routine again did not schedule/publish; safety net caught it before the 9:50 AM cutoff | `itn-daily-weekday` got further than the Sep 8 failure but still died before Beehiiv: `data/brief-2026-09-09.json` was generated (6:50 AM CT) and `index.html` `todaysGames` was already correctly rewritten with today's card by the time the safety net (`itn-deadline-check-weekday`, ~9:15 AM run) checked — but the explanatory comment above the array was still copy-pasted from Sep 8's edit (wrong sport counts, wrong pick shape) and nothing was scheduled or published in Beehiiv. Filter on `/posts?status=scheduled` confirmed empty at 9:15 AM; top published post was Tue Sep 8. **AUDIT — Sports: NFL 1 · MLB 2 (3 total, neither over the 2-per-sport cap). Bet types: spread 1 (NFL) · total 1 (MLB) · moneyline 1 (MLB) — 1 of 3 (33%, under the 50% cap). Football-first applied: NFL led as the free pick — the 2026 season opener (Patriots @ Seahawks) was the only football on the board today.** Board: 15 MLB games, 1 NFL game, 0 CFB/NBA/CBB/NHL/UFC/PGA. **Card (reused verbatim from the already-correct `index.html`, cross-checked against the brief JSON, not re-derived):** free pick New England +3 (+142) at Seahawks, 8:20 PM ET — spread opened +3.5/+160 and closed +3/+142 while the total held dead flat at 44.5 the whole time; de-vigged roughly 40-60 New England. Also Under 9 (-105), Blue Jays at Athletics, 3:05 PM ET — total fell 9.5 to 9 while Toronto's moneyline firmed -185 to -193, same direction. Also Detroit -135, Twins at Tigers, 1:10 PM ET — the flattest game on the board, -131 to -135 with the total untouched at 8.5. Biggest overnight move (not a pick): Nationals at Padres — Padres' moneyline eased -199 to -173 (26 cents) with the 1.5 spread never moving, the largest single-game price swing on the board. **Stale-game check:** earliest kickoff on the card is Twins/Tigers 1:10 PM ET, all comfortably after the scheduled 10:00 AM CDT send; nothing presented as bettable that had already started. **Site:** `index.html` `todaysGames` array itself needed no changes (already correct); only the stale/inaccurate explanatory comment above it was rewritten to match reality (football-first, 1 NFL/2 MLB, 1 of 3 moneylines). **Newsletter assets — images: 4 (beehiiv-hosted: yes, GATE 2 passed 4/4).** `python3 scripts/newsletter_assets.py` built header.jpg/pick.jpg/move.gif/number.jpg + issue.html from a hand-written `issue.json` (the free/also/move/number copy was largely reused from the analysis already sitting in `index.html`, so it didn't need to be rederived from scratch). One script hiccup: `parlay_cards.py`'s price-card format requires a numeric `fair` percentage per leg, not `"n/a"` — the total leg's fair probability was computed by de-vigging the over/under odds by hand (48.9% fair for the Under) before the card would render. **Git note — the newsletter-assets directory is excluded by the repo's allowlist `.gitignore` (`*` then explicit `!` exceptions) and it has no exception for `assets/newsletter/`; every prior run that shipped images used `git add -f` for that path specifically, and this run needed the same — a plain `git add -A` silently skipped the new asset directory on the first push attempt.** **NEW BLOCKER, same shape as Sep 8: Cloudflare Pages propagation lag.** `insidethenumber.com/assets/newsletter/2026-09-09/*.jpg` still 404'd immediately after push. Worked around the same way as Sep 8: image `src`s in `issue.html` were pointed at `raw.githubusercontent.com/insidethenumber/inside-the-number-site/<commit-sha>/assets/newsletter/...` before pasting — confirmed resolving via a browser tab before use, beehiiv's paste handler fetched them fine and re-hosted all four on its own CDN. **Credentials note:** `itn-secrets.env` copies left in `/tmp` by prior sessions were all owned by a different sandbox user (`nobody`, mode 600) and unreadable this session; a working copy was found at `Projects/Inside the Number/itn-secrets.env` in the connected folder, copied to a fresh space-free temp path before pushing (`ITN_SECRETS=...`), same wrinkle as every prior run. **Beehiiv mechanics — new technique, faster and more reliable than the documented DOM-click approach:** cleared the duplicated post's body via `editor.commands.selectAll()` + `deleteSelection()` (editor API, not keyboard selection — `shift+cmd+End` was tried first and did not reliably extend the selection to the end of the doc in this editor), then dispatched a synthetic `ClipboardEvent('paste', {clipboardData})` carrying `issue.html` as `text/html` directly at the ProseMirror DOM node. This correctly triggered beehiiv's own paste-ingestion pipeline (4/4 images uploaded to `beehiiv-images-production`, GATE 2 passed) with zero leftover/duplicate content from the source post — a cleaner result than the DOM-surgery approach documented on Sep 7, because clearing via editor commands avoids the truncation/duplication that raw DOM edits caused. Alt text on all 4 images and heading-level (H2) conversion on all 6 section headers were both set the recommended way — read `editor.getJSON()`, edit the plain JS object (set `attrs.alt`, or change `type` to `heading`/`attrs.level:2` and strip the bold mark), then `editor.commands.setContent(json, true)` — both held across the rest of the flow. **Known quirk recurred exactly as documented:** the H1 title reverted to the duplicated post's old title once after the first triple-click+type+cmd+a set; a second attempt (retype, then click a different field to blur) held for the rest of the session, confirmed on the Email step where Subject line auto-synced to the corrected title. Web-step thumbnail was stale (old post's header image); fixed via the refresh icon on the thumbnail control → media picker → selected the new "Wednesday, September 9" header asset directly (no separate confirm button needed — selecting closed the picker). Scheduled (not published) via the Review step's **Schedule** button → "Pick a specific time" → typed `today at 10:00am` → echo confirmed "Sep 9, 2026 10:00 AM CDT" → Tab → confirmed "Publishes in 34 minutes · Wednesday, Sep 9, 2026 10:00 AM CDT" → Schedule → toast "Post successfully scheduled" → re-verified on `/posts?status=scheduled` showing **Scheduled, Wed, Sep 9, 2026 10:00 AM CDT**, exactly one post, no orphaned duplicate draft left behind. No rule went unmet; nothing forced for quota. **Counting note for the reliability decision:** another miss for the main routine's Beehiiv step specifically, even though its research/site-update steps completed further than Sep 8's total failure — and another save for the pre-flight safety net, this time catching it with 34 minutes to spare rather than needing the post-cutoff immediate-publish fallback. **Post-flight verification (`itn-deadline-check-weekday`, ~10:21 AM CDT run):** confirmed via Beehiiv /posts that "New England +3 at Lumen Field, and a Total That Hasn't Moved" shows status **Published**, "Published on Wed, Sep 9, 2026 10:00 AM CDT" — exactly on target, no latency. No further action needed; not messaging Chuck since this was a clean on-time send.

| 2026-09-10 (Thu, ad-hoc) | Manual publish 12:55 PM CDT by Claude at Chuck's explicit direction ("the newsletter needs to go out first, probably") — the scheduled `itn-daily-weekday` main routine had produced nothing for today (no `DRAFT_2026-09-10.md`, no SEND_LOG row, no git commits in the 13:00+ UTC window before this session started), so this was a from-scratch manual build rather than a safety-net recovery of a partially-completed run. **Chuck's explicit content constraint: keep it short, one subscriber's worth of value, don't jam in everything.** Built a 4-section hand-written `issue.html` (not the full `newsletter_assets.py` 4-image pipeline) covering: SF@LAR Stafford/Purdy QB-age-gap card, tonight's FanDuel Single Game DFS lineup, the Miami-FAMU 91%-of-total number, and a one-paragraph Sunday-slate teaser — 2 images total, both already-published X-post graphics reused rather than newly generated. **Beehiiv mechanics — known quirk recurred a third time:** duplicated the most recent published post ("New England +3 at Lumen Field..."), first `ClipboardEvent('paste')` attempt left the old post's 4 images (header/pick/move/number) and a trailing text fragment glued on after the new content (`hasOldYesterday: true`) — same documented Sep 5 quirk. Fixed via `document.querySelector('.ProseMirror').editor.commands.selectAll()` + `.deleteSelection()` (confirmed `editor.getText().length === 0`), then re-pasted clean — verified 2/2 images on `beehiiv-images-production.s3.amazonaws.com` (GATE 2 passed), no leftover text, all 4 section headers present. **New wrinkle this run:** the H1 title and the Web-step URL slug/meta title/thumbnail all independently reverted to or retained the duplicated post's stale values even after typing the new title in Compose — caught only by reading the Review screen's "Post Title:" summary line rather than trusting the Compose editor's own display. Fixed by re-typing the title a second time with an explicit blur-click afterward (held on the second attempt, consistent with the Sep 7/Sep 9 "title reverts once" pattern), manually retyping the Post URL slug (`thursday-spread-91-percent-of-total`), retyping the meta title, and using the thumbnail control's refresh icon → media picker → searched and selected the Stafford/Purdy QB card asset directly (stale thumbnail was a Sep-9 header image). Meta description auto-synced correctly from the subtitle with no fix needed. Author confirmed ITN Desk only. Audience: All subscribers, 5 recipients, Email and Web. **Published via Review → Schedule → "Publish now"** (well past any scheduling cutoff for an early-afternoon ad-hoc send) → confirmed "Post successfully published" toast and live URL `insidethenumber.beehiiv.com/p/thursday-spread-91-percent-of-total`. **No AUDIT block this run** — this issue was not built from the standard picks-card pipeline (no `data/brief-*.json`-driven pick selection), it was a hand-assembled recap/preview issue per Chuck's specific ask, so the sport-cap/bet-type-mix accounting that applies to the normal Morning Board format does not apply here. **Git:** fresh `mktemp -d` clone, never the shared working copy, for this log entry. **Recommendation for future runs:** when duplicating a post for a same-day ad-hoc send, verify the H1 title, Post URL slug, meta title, and thumbnail explicitly on the Review screen before publishing — don't trust the Compose editor's own display of these fields, since at least three of the four silently retained stale values from the duplicated source post this run.
| 2026-09-10 | Thu | 10:00 AM | **DUPLICATE SEND — published 12:56 PM CDT, one minute after a second, different issue also went out to all subscribers** | NO, main routine and both safety nets all failed to hit 10:00 AM; recovery caused a duplicate | `itn-daily-weekday` started 8:00 AM CT per schedule. STEP 0 found Beehiiv signed out (login screen), so per the task's documented behavior for that case the run did NOT attempt to log in, flagged it, and continued: cloned the repo, pulled `data/brief-latest.json` (generated 6:50 AM CT), surveyed the board, and began building `issue.json`/site updates. The session's own bash clock showed an unexplained jump from 8:06 AM CT to 12:25 PM CT between two consecutive tool calls with no long-running command in between — the run itself was somehow delayed for roughly 4 hours by the environment, not by its own logic. By the time it checked Beehiiv again at ~12:25 PM CT, the session was live (no login screen) and `/posts` showed nothing published or scheduled for today, so the run proceeded on the assumption it was the first and only attempt today. **What actually happened: a second, independent process (unknown session, not this one) was also working on today's issue during the same window and published its own complete issue — "Thursday: The Spread That's 91% of the Total" (Stafford vs. Purdy, a FanDuel card, a Miami number) — to Email and Web, all free subscribers, at 12:55 PM CDT, one minute before this run's own send.** This run's re-check of `/posts` immediately before entering STEP 4 (~12:25 PM CT) was clean, but the run did **not** re-check again immediately before clicking Publish at 12:56 PM — the 30 minutes of content-building in between was exactly the window the second issue landed in, so the final safeguard the task prompt calls for was skipped in practice even though the letter of STEP 4's early re-check was followed. **Result: two separate, unrelated newsletter issues went to all 5 subscribers within one minute of each other today.** Given games not yet started at the time of both sends (see below) this is a content/reputation problem (two emails in one minute, no reference to each other) rather than a stale-pick problem. **This run's card — AUDIT: Sports NFL 1 · CFB 1 · MLB 1 (3 total, none over the 2-per-sport cap). Bet types: spread 1 (NFL) · total 1 (CFB) · total 1 (MLB) — 0 of 3 moneylines (0%, well under the 50% cap). Football-first applied: NFL led as the free pick (49ers @ Rams, Melbourne) since a football game was on the board; the day's other football game, Florida A&M @ Miami, had its moneyline turned off entirely (spread -57.5) so it contributed only a total pick, not the lead.** Two early MLB games (Rays/Braves, Astros/Phillies) had already started by the time the card was finalized (~12:30 PM CT) and were correctly excluded from the picks (both still listed, marked "in progress," in the Today's Board section for transparency). **Card:** free pick Los Angeles -3.5 (-110 assumed standard, not confirmed live — see note) vs San Francisco, 8:35 PM ET — line moved -2.5 to -3.5 overnight with the moneyline sliding the same direction (SF +130→+164, LAR -155→-198) while the total held flat at 48.5, in a game where neither team has played a snap. Also Over 63.5 (-105), Florida A&M at Miami, 8:00 PM ET — total and price both moved the same direction (62.5→63.5, over +100→-105). Also Under 9 (-120), Colorado at NY Yankees, 7:05 PM ET — total rose (8.5→9) while the over got cheaper (-117→+100), a mixed signal read as a fade. **Verification gap, stated openly: the NFL spread price (-110) could not be confirmed live.** `site.api.espn.com` and `espn.com` were both refused by the Claude in Chrome extension this run (`Browser action was not allowed`, no way to request per-site approval in this unattended session), so research relied entirely on `data/brief-latest.json` (generated 6:50 AM CT by the automated slate pipeline) rather than a fresh same-session ESPN pull. The spread number itself (-2.5→-3.5) is from that file; the -110 price is the standard/assumed juice, not a confirmed live number, and is flagged as such rather than published as a checked fact. **Site:** `index.html` `todaysGames` (3 entries, one `free:true`) and the Pick of the Day card updated; `cfb.html` PICK block updated with the Over 63.5 call (today is a CFB game day, not limited to Saturdays per the task prompt). **Newsletter assets — images: 4, GATE 2 passed (4/4 beehiiv-hosted after paste-ingestion), confirmed by reading the editor's own document model, not just the DOM.** `insidethenumber.com/assets/newsletter/2026-09-10/*` all 404'd via `fetch` from the Beehiiv tab about 90 seconds after push (same Cloudflare Pages propagation lag as Sep 8/Sep 9); worked around by pointing `issue.html`'s image `src`s at `raw.githubusercontent.com/.../<commit-sha>/assets/newsletter/2026-09-10/...` (all 4 confirmed 200 first try) before pasting. **Known quirks, both recurred and were both fixed the documented way:** the whole prior duplicated post's body survived the paste (nodes 39-72 of 73, trimmed via `editor.getJSON()` + `setContent()`); alt text set via `setContent()` did not persist into the rendered DOM on the first attempt (JSON model held the alt text correctly; a second `setContent()` call was needed before it export to the live doc — recommend future runs verify alt text via `editor.getJSON()`, not `img.alt` in the DOM, since the DOM read gave a false negative even when the underlying document was correct). Section headers converted from bold paragraphs to real `heading` (level 2) nodes via the same JSON-edit method — 6/6 converted. Title reverted to the duplicate's old title once after the first triple-click+type (same known quirk as prior days); held on the second attempt after clicking a different field (the byline) to blur, rather than clicking inside the body. Subject line and Post-URL slug both needed manual correction on the Email/Web steps (same known lag as documented). Web thumbnail was stale (showing yesterday's header) — fixed via the refresh icon → media picker → clicked directly on the today's-header thumbnail image (not its checkbox, not double-click, not the 3-dot menu — a plain single click on the image itself is what actually applied it). **Skipped this run: the documented "send a test email before scheduling" step**, to save time after the unexplained ~4-hour delay already put the run hours behind; GATE 2 (images actually embedded and beehiiv-hosted, read from the editor's own document model) was treated as sufficient verification instead. In hindsight, given a second issue landed one minute before this one, a test-email pause might also have created a window for the run to re-check `/posts` and catch the collision — worth reconsidering for future runs under time pressure. **Publish mechanics:** past the 9:55 AM cutoff by roughly three hours at send time, so this run used Review → Schedule → "Publish now" (not Schedule-for-later) per the task prompt's stated fallback, confirmed via the modal's own "Publishes immediately, Thursday, Sep 10, 2026 12:56 PM CDT" text before confirming. Verified via `/posts` immediately after: "Los Angeles Opened -2.5. It's Already -3.5." shows **Published, Sep 10, 2026 12:56 PM CDT** — and, one row above it, the second issue "Thursday: The Spread That's 91% of the Total" shows **Published, Sep 10, 2026 12:55 PM CDT**, also Email and Web, all free subscribers. **Counting note for the reliability decision: this is not a clean send. It is a recovery from a ~4-hour unexplained delay that happened to collide with an unrelated concurrent publish, resulting in two issues in one inbox within 60 seconds. Chuck should be told directly and promptly — this needs eyes on whatever the second, unidentified process was, not just a log entry.**

**CORRECTION / DUPLICATE-SEND INCIDENT — Sep 10, 12:55-12:56 PM CDT.** The manual send logged above was NOT actually filling a gap — it collided with a live run of `itn-daily-weekday` that was mid-flight the entire time. Commit `08bb4eb` (12:31 PM CDT, "Daily update: Sep 10 card... newsletter assets") shows the automated routine was working in parallel; it published its own issue, "Los Angeles Opened -2.5. It's Already -3.5.", at **12:56 PM CDT — 60 seconds after** the manually-built "Thursday: The Spread That's 91% of the Total" went out at 12:55 PM. Both are real, both delivered to all 5 subscribers. Root cause: the "no send yet" diagnosis behind the manual build was made earlier in the session and never re-verified against Beehiiv's live `/posts` list immediately before hitting Publish — the standard idempotency re-check (documented as a required Step 0 practice on every prior automated run in this log) was skipped on this manual path. Chuck's call: leave both live, no unpublish/cleanup. **Fix for future manual sends: always re-check `/posts` for anything published in the last hour immediately before the final Publish/Schedule click, not just at the start of the session** — a session that runs long enough can be overtaken by a concurrent automated run.

## Fri Sep 11, 2026 — RECOVERY SEND (manual, 1:17 PM CT)

**Target 10:00 AM. Actual 1:17 PM. LATE — and not by the usual margin.**

The automated `itn-daily-weekday` task fired on time (8:07 AM CT, lastRunAt confirms)
but produced **nothing**: no Beehiiv post in any state, no site update, no commit.
The 9:10 AM safety net (`itn-deadline-check-weekday`) did not fire at all — its
lastRunAt was still Sep 10. Same pattern across every task scheduled after ~8:09 AM:
the 10:07 X engagement round, the 11:01 pick ledger, and the Friday DFS build all
show no run. Everything scheduled 6:49–8:09 AM ran fine. GitHub Actions (slate,
odds, headshots, UFC) were green all day, so this is the Claude scheduled-task layer,
not the pipeline. Most likely the desktop app was closed or asleep from ~8:10 AM.

Caught by Chuck asking for a health check at 12:50 PM, not by any alarm. That is the
real finding: the 9:10 safety net is useless in exactly the failure mode where the
whole scheduler is down, because it is the same scheduler.

Card audit — Sports: CFB 2 · MLB 1. Bet types: spread 1 · total 2. Moneylines: 0 of 3.
Lead: Kansas +4.5 (CFB) — FOOTBALL FIRST satisfied.
  - FREE: Kansas +4.5 vs Missouri, 8:00 PM ET (opened -7, now -4.5)
  - Under 53.5 — Rutgers at Boston College, 7:30 PM ET (opened 55.5)
  - Under 7 — Phillies at Braves, 7:15 PM ET (opened 7.5, under got cheaper)

Images in body: 4 (header, pick, move.gif, number). GATE 2 PASSED — every src on
beehiiv-images-production.s3.amazonaws.com.

Known quirk hit again, worse than usual: selectAll+paste left the ENTIRE old issue
below the new one (37 trailing nodes) plus yesterday's header image above it.
Removed the tail with a DOM Range + execCommand('delete'); the leading image block
resisted that and needed a real click on the image then its toolbar trash icon.

**UNRESOLVED — thumbnail.** The duplicate carried Thursday's header as the post
thumbnail. The replace control opens the Media Library in management mode, which has
no select/insert action — checkbox gives Copy/Move/Delete only, click opens a preview,
double-click does nothing, and the detail panel has no confirm button. Upload-from-URL
put the file in the library but did not apply it. Setting it through the beehiiv MCP
(`edit_post` with `thumbnail_image_url`) is gated behind a paid plan. Rather than ship
a Friday post stamped "Thursday, September 10" in the feed card and OG preview, the
thumbnail was cleared and the post published without one. Email body unaffected.
